...
After showing that \\$1 of transfers from this program lead to about \\$1 of additional spending, Feiveson breaks down spending into four categories: wages, employment, capital outlays, and everything else. The paper then interacts the amount of federal transfers with an indicator for whether the state requires collective bargaining or not. The key results appear in Table 9 and are copied below:
...
So far, so good. But how do public sector unions fit it here? Diamond repeats the empirical exercise from them, replacing their amenities variables with a measure of housing supply elasticity based on the availability of buildable land from Saiz (2010).
